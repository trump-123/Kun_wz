...
function LEADER_STOP_POS()
	...
	return ball.pos() + Utils.Polar2Vector(-530, idir)
end
...
function refStopAroundBall()
	local BLOCK_DIST = param.freeKickAvoidBallDist + 2 * param.playerRadius
	local AWAY_DIST = 25 + param.playerRadius
	...
	local SIDE_POS = function()
		return ball.pos() + Utils.Polar2Vector(BLOCK_DIST, dir.ballToOurGoal() + BLOCK_ANGLE)
	end
	...
	local SIDE2_POS = function()
		return ball.pos() + Utils.Polar2Vector(BLOCK_DIST, dir.ballToOurGoal() + 2*BLOCK_ANGLE)
	end
...
function backBall(p)--站在距离目标点到球向量方向180mm的位置
	return function ()
...
function testTwoKickOffPos2()--
	local BLOCK_DIST = 30
	local AWAY_DIST = 7.5 + param.playerRadius--半径
	local BLOCK_ANGLE = math.asin(AWAY_DIST / BLOCK_DIST)*2
	local factor = ball.antiY
	return function()
...
function chipPassForTouch(p)--点p到球门方向100mm的位置，暂时还不知道具体要作什么
	local length = 100
	--local length = 9.33 -- 2017年伊朗赛前测量
	-- local length = 9.33+2 --gty改
	if type(p) == "function" then
...
function passForHead(p)--我感觉是获取传到机器人嘴里的球
	if type(p) == "function" then
		return function ()
			return p() + Utils.Polar2Vector(100, dir.posToTheirGoal(p()))
		end
	else
		return p + Utils.Polar2Vector(100, dir.posToTheirGoal(p))
	end
...
-- 若pos为函数,dir最好也是函数
function disappearPos(pos, dir, i)--返回pos点向场内运动15mm的点
	if i == -1 then
...
function getBackPos()--要么到球上下半场反向 要么到球左右半场反向
	local bPos = function ()
		local bx
		local by
		if ball.posX()> 0 then
...
function reflectPassPos(role)--没用
	return function ()
...
function reflectTouchPos(role, y)--没用
	return function ()
