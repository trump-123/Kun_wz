...
function toPlayerHeadDir(recver)
    return function (passer)
    	local tmpPlayerHead = player.pos(recver) + Utils.Polar2Vector(param.playerFrontToCenter, player.dir(recver))
...
function syntYPos(p)--与球同向
    return function ()
    	return CGeoPoint(p:x(), syntY()*p:y())
    end
end

function toFuncDir(f)--到function点的dir 没有反算
    return function ()
    	return (f() - pos()):dir()
    end
end

function antiYDir(p)--到目标点的dir 含反算
    return function ()
    	if type(p) == "userdata" then
...
function goRush(deltaX,Y)--定位球跑位
...
function goRush4kicker()--定位球定点站位
    return function ()
    	-- by gty
...
function cornerStay(toballdist,dir,step,num,p)--角球站位
    return function ()
    	if p == nil then
    		if antiY() == 1 then
...
function waitChipPos()--等待接挑球的位置
    return function()
    	return CGeoPoint:new_local(refPosX() + 100,220 * refAntiY())
    end
end

function backDir(p, anti)--球到点p向量角度
    return function ()
    	local idir
    	if type(p) == "function" then
    		idir = p()
    	...
    	elseif type(p) == "userdata" then
    		if anti == false then
    			idir = Utils.Normalize((p - ball.pos()):dir())--球到p点向量角度 没有反算
    		else
    			local targetP = CGeoPoint:new_local(p:x(), ball.antiY()*p:y())--球到p点向量角度 含反算
    			idir = Utils.Normalize((targetP - ball.pos()):dir())
    		end
    	elseif type(p) == "string" then
    		idir = Utils.Normalize((player.pos(p) - ball.pos()):dir())--到球员的向量角
    	end
...
function backPos(p, d, s, anti)
    return function ()
    	...
    	local shiftVec = Utils.Polar2Vector(s, idir):rotate(syntY()*math.pi/2)--绕着点旋转90度
    	local ipos = ball.pos() + shiftVec + Utils.Polar2Vector(d, Utils.Normalize(idir + math.pi))--横向偏移加纵向偏移
    	return ipos
    end
end
...
function jamPos(p, d, s)
    return function ()
    	...
    	local shiftVec = Utils.Polar2Vector(s, faceDir):rotate(antiY()*math.pi/2)--总是在向球的内侧
...
function jaminner(dist)
    return function ()
    	...
    	local balltoGate = gate - ball      --球到球门的向量
    	local d = dist and dist or 580      
...
function updateRef2PlacePos()
    ...
    if dist > 15 then
        gRefMsg.ballX = placementPos():x()  --nil false
        gRefMsg.ballY = placementPos():y()
        gRefMsg.antiY = placementPos():y()>0 and -1 or 1 --placementPos():y()>0为真返回1，为假返回-1
    else
        gRefMsg.ballX = posX()
        gRefMsg.ballY = posY()
        gRefMsg.antiY = antiY()
...
function chipFixBuf(chipPower) --挑球fix帧数，适用于17年底盘，距离为1米到5米 2017.7.26
    local fixBuf = (-0.0000015498 * chipPower * chipPower + 0.0025344180 * chipPower + 0.2463515283) * param.frameRate
    return fixBuf
end

function getFixBuf(p)--挑球帧数，挑球需要花费多少时间
    if type(p) =="function" then
    	p = p()
    end
...
function chippassvel(p)
    local ball = pos()
    local CHIP_DIST_RATIO = 0.8
    local passvel = 0
    local ipos
         if type(p) == "function" then
            ipos = p()
    ...
    local distance = dist(ipos)
    local chipdist = distance * CHIP_DIST_RATIO
    passvel = chipdist / 1.266
    if passvel > 4000 then
        passvel = 4000
    end
    if passvel < 500 then
        passvel = 500
...
function toBestEnemyDist()--球到敌方对我方最具威胁的地方
...
function enemyDistMinusPlayerDist(role)--敌方最具威胁的机器人到球的距离减去我方机器人到球距离
