...
function SelectRefPlay()
	local curRefMsg = vision:getCurrentRefereeMsg()--vision:getLuaRefereeMsg()
...
if SelectRefPlay() then
	...
	elseif IS_TEST_MODE then
		...
		--add by twj 14/5/12
		------------------------------------------
		if gNormalPlay == "NormalPlayPass" or gNormalPlay == "NormalPlayPP" or gNormalPlay == "NormalPlayMessi"
			or gNormalPlay == "NormalPlayMessi_6vs6" or gNormalPlay == "NormalPlayMessi_8vs8" or gNormalPlay == "NormalPlayDefend" then
			--world:setBallHandler(gRoleNum["Kicker"])
