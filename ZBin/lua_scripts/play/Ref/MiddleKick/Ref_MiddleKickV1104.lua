...
local Third  = ball.refAntiYPos(CGeoPoint:new_local(pl/2 - ped-500,pew/2+500))
local Fourth = ball.refAntiYPos(CGeoPoint:new_local(pl/2 - ped-1000,-200))
local Fifth  = ball.refSyntYPos(CGeoPoint:new_local(pl/2 - 1500, pw/2 - 1500))
local Sixth  = ball.refAntiYPos(CGeoPoint(920,1105))
...
gPlayTable.CreatePlay{
  ...
  ["start"] = {
    ...
    Fronter  = task.goCmuRush(Fifth ,player.toPointDir(ToGoalPoint[2]),_,DSS_FLAG + flag.free_kick),
    Kicker   = task.goCmuRush(Sixth ,player.toPointDir(ToGoalPoint[2]),_,DSS_FLAG + flag.free_kick),
    Center   = task.wback(3,1),
