...
local our_player_stand_pos = {
      ...
      CGeoPoint:new_local(-param.pitchLength/2 + param.penaltyDepth + 500, 1700),
      CGeoPoint:new_local(-param.pitchLength/2 + param.penaltyDepth + 500, 2100),
      CGeoPoint:new_local(-param.pitchLength/2 + param.penaltyDepth + 500,-2100),
      CGeoPoint:new_local(-param.pitchLength/2 + param.penaltyDepth + 500, 2500),
...
["WaitStart"] = {
  ...
  Kicker   = task.goCmuRush(our_player_stand_pos[8],0,_,DSS_FLAG),
  Fronter  = task.goCmuRush(our_player_stand_pos[9],0,_,DSS_FLAG),
  Center   = task.goCmuRush(our_player_stand_pos[10],0,_,DSS_FLAG),
  Goalie   = task.goalie(),
  match = "[LASMDBRKFC]"
},
...
["PenaltyKick"] = {
  switch = function()
    ...
    --  -- return "exit"
    -- end
    -- if  bufcnt( player.kickBall("Leader"), "", 750) then
    --   return "exit"
    -- end
  end,
  Leader   = task.bigpenaltykick(),
  Assister = task.goCmuRush(our_player_stand_pos[1],0,_,DSS_FLAG),
  Special  = task.goCmuRush(our_player_stand_pos[2],0,_,DSS_FLAG),
  Defender = task.goCmuRush(our_player_stand_pos[4],0,_,DSS_FLAG),
  Middle   = task.goCmuRush(our_player_stand_pos[5],0,_,DSS_FLAG),
...
name = "Ref_BIGPenaltyKick_11vs11",
applicable ={
	exp = "a",
	a = true
},
