...
local OUR_NOT_DSS = flag.not_avoid_our_vehicle + flag.dodge_ball + flag.free_kick
local PASS_ACC    = 2000
...
--6*4
local First  = ball.refAntiYPos(CGeoPoint:new_local(pl/2-ped+100,pew/2+350))
local Second = ball.refSyntYPos(CGeoPoint:new_local(pl/2-ped-300,pew/2))
local Third  = ball.refSyntYPos(CGeoPoint:new_local(pl/2-ped-600,pew/2-200))
local Fourth = ball.refSyntYPos(CGeoPoint:new_local(pl/2-ped-900,pew/2-400))

local Real = ball.refSyntYPos(CGeoPoint:new_local(2250,670))

local Real1 = ball.refSyntYPos(CGeoPoint:new_local(2250,670-800))
...
local ANTIPos  = ball.refAntiYPos(CGeoPoint:new_local(pl/8,0))
...
gPlayTable.CreatePlay{
  ...
  ["start"] = {
    ...
    Assister = task.goCmuRush(First ,player.toPointDir(ToGoalPoint[3]),_,DSS_FLAG + flag.free_kick),
    Special  = task.goCmuRush(Second,player.toPointDir(ToGoalPoint[2]),_,DSS_FLAG + flag.free_kick),
    Middle   = task.goCmuRush(Third ,player.toPointDir(ToGoalPoint[1]),_,DSS_FLAG + flag.free_kick),
    Defender = task.goCmuRush(Fourth,player.toPointDir(ToGoalPoint[1]),_,DSS_FLAG + flag.free_kick),
