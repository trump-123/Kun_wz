...
switch = function()
	...
	-- 		return "finish" and ball.posX() > -1/6 * param.pitchLength
...
["attacker6"] = {
	Leader   = task.marking(enemy.nearest1,true,5000),
	Assister = task.wmarking("Zero",_,getAttackerNum(0)),
	Special  = task.wmarking("First",_,getAttackerNum(1)),
	Middle   = task.wmarking("Second",_,getAttackerNum(2)),
	Defender = task.goSpeciPos(Defence_Pos,player.toBallDir,flag.allow_dss + flag.avoid_stop_ball_circle),
	Breaker  = task.wback(2,1),
	Receiver = task.wback(2,2),
	Goalie   = task.goalie(),
