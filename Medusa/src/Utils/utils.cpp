...
//            return p;
    ...
    CGeoPoint MakeOutOfOurPenaltyArea(const CGeoPoint& p, const double buffer,const double dir) {
        // rectangle penalty
        if(WorldModel::Instance()->CurrentRefereeMsg() == "OurBallPlacement")
            return p;
        ...
        if(fabs(dir) < 1e4) {
            ...
            while(InOurPenaltyArea(newPoint, buffer) && newPoint.x() > -PARAM::Field::PITCH_LENGTH / 2)
                newPoint = newPoint + adjustVec;
