...
struct VehicleInfoT {
    ...
    CVector accelerate;
    float imu_dir;
    float imu_rotate_vel;
    int type;
};
enum ballState {_touched, _shoot, _struggle, _pass, _dribble, _stop, _outoffied, _control, _undefine, _kicked, _chip_pass, _flat_pass,_uncertain_pass};

struct VisualInfoT {
    VisualInfoT(): cycle(0), BallState(_dribble), BallLastTouch(0), mode(0) {}
    unsigned int cycle;
    VehicleInfoT player[PARAM::TEAMS][PARAM::Field::MAX_PLAYER];
    PosT ball;
    PosT rawBall;
    PosT chipPredict;
    PosT bestChipPredict;
    PosT secondChipPos;
    double chipFlyTime;
    double chipRestFlyTime;
    CVector3 realPos;
    CVector3 chipVel;
    CVector BallVel;
    CVector BallRawVel;
