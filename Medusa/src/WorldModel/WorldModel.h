...
public:
    ...
    int getEnemyAmountInArea(double x1, double x2, double y1, double y2, std::vector<int>& enemyNumVec, double buffer = 0);
    int getEnemyAmountInArea(const CGeoPoint& center ,double radius, std::vector<int>& enemyNumVec, double buffer = 0);
    CGeoPoint penaltyIntersection(CGeoSegment s);
    void normalizeCoordinate(CGeoPoint& p);
    double preditTheirGuard(const PlayerVisionT& enemy, CGeoPoint& p);
    double preditOurGuard(const PlayerVisionT& our, CGeoPoint& p);

    void setBPFinished (bool bpFnished) { BPFinished = bpFnished; }

    bool getBPFinished() {return BPFinished;}
private:
    bool BPFinished;
