...
//		int attcnt = getAttackNum();
        attackInform.append(QString("R%1:").arg(getAttackNum()));
        for(int i=0;i<getAttackNum();i++)
            attackInform.append(QString(" %1").arg(QString::number(_attackArray[i], 16).toUpper()));
        QString attackSteadyInform;
        attackSteadyInform.append(QString("S%1:").arg(getAttackNum()));
        for(int i=0;i<getAttackNum();i++)
            attackSteadyInform.append(QString(" %1").arg(QString::number(_steadyAttackArray[i], 16).toUpper()));
        GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(1000,-PARAM::Field::PITCH_WIDTH / 3),attackInform.toLatin1(),COLOR_YELLOW);
        GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(1000,-PARAM::Field::PITCH_WIDTH / 3 - 150),attackSteadyInform.toLatin1(),COLOR_YELLOW);
        for(int i = 0;i<PARAM::Field::MAX_PLAYER;++i)
        {
