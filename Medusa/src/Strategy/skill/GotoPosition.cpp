...
CPlayerCommand* CGotoPositionV2::execute(const CVisionModule* pVision)
{
    ...
    //GDebugEngine::Instance()->gui_debug_arc(pVision->ball().RawPos(),500,0.0,360.0,COLOR_ORANGE);
    GDebugEngine::Instance()->gui_debug_line(self.Pos(), target, TASK_TARGET_COLOR);
    //GDebugEngine::Instance()->gui_debug_line(self.Pos(),(self.Pos() + Utils::Polar2Vector(2000,self.Dir())),4);
    ...
    if(needBreakRotate) {
        ...
        GDebugEngine::Instance()->gui_debug_line(vecPos,vecPos+Utils::Polar2Vector(300,dir),0);
        if(dir < 1e4){
            final.SetDir(dir);
            //use_dir = false;
        }
