...
void CTier::plan(const CVisionModule* pVision){
    ...
    if(stand_intersection.intersectant()){
        if(ball.dist2(stand_intersection.point1()) < ball.dist2(stand_intersection.point2()))
            stand_pos = stand_intersection.point1() + Utils::Polar2Vector(PARAM::Field::MAX_PLAYER_SIZE/2+10,Utils::Normalize(get_defence_direction(ball)+pi*3/2));
        else
            stand_pos = stand_intersection.point2() + Utils::Polar2Vector(PARAM::Field::MAX_PLAYER_SIZE/2+10,Utils::Normalize(get_defence_direction(ball)+pi*3/2));
...
//    {
    ...
    //}
       ...
       if(self.X() > -PARAM::Field::PITCH_LENGTH/2 + PARAM::Field::PENALTY_AREA_DEPTH + 500)
       {
           taskFlag = PlayerStatus::ALLOW_DSS;
       }
