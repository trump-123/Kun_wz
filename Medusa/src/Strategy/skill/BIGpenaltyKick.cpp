...
namespace {
    ...
    CGeoPoint RIGHT_GOAL(PARAM::Field::PITCH_LENGTH/2,PARAM::Field::GOAL_WIDTH/2 - 200);
    CGeoPoint LEFT_GOAL(PARAM::Field::PITCH_LENGTH/2,-PARAM::Field::GOAL_WIDTH/2 + 200);
...
CBIGpenaltyKick::CBIGpenaltyKick()
    :goBackBall(true)
    ,turnPos(true)
    ,canKick(false)
    ,isKick(false)
...
void CBIGpenaltyKick::plan(const CVisionModule* pVision){
    ...
    CVector me2right = RIGHT_GOAL - self.RawPos();
    CVector me2left = LEFT_GOAL - self.RawPos();
    CVector me2mid = MIDDLE_GOAL - self.RawPos();
    CVector ball2mid = MIDDLE_GOAL - ballPos;
    ...
    /**********        set subTask       ************/
    if(goBackBall){
        CGeoPoint gotoPoint = ballPos + Utils::Polar2Vector(-500,ball2mid.dir());
        setSubTask(PlayerRole::makeItGoto(vecNumber,gotoPoint,me2ball.dir(),PlayerStatus::DODGE_BALL| PlayerStatus::NOT_AVOID_THEIR_VEHICLE | PlayerStatus::NOT_AVOID_PENALTY));
        ...
        if(fraredOn > getballTime){
           ...
           double taskDir;
           CGeoPoint target = MIDDLE_GOAL;
           ShootModule::Instance()->generateBestTarget(pVision,target,self.Pos());
           if(ballPos.x() > PARAM::Field::PITCH_LENGTH/4 ||(shootTime>(PARAM::Vision::FRAME_RATE +5)* 8 )){
                taskDir = (target - self.Pos()).dir();
           }else{
                taskDir = me2mid.dir() - self.Dir();
           }
           if(abs(taskDir - self.Dir()) < 5*PI/180 ||(shootTime>(PARAM::Vision::FRAME_RATE +5)*8.5 )){
               canKick = true;
               notDribble = true;
           }
           notDribble = false;
           setSubTask(PlayerRole::makeItGoto(vecNumber,getballPos,taskDir,PlayerStatus::NOT_AVOID_THEIR_VEHICLE|PlayerStatus::NOT_AVOID_PENALTY));

        } else {
            notDribble = false;
            CGeoPoint getballPos = ballPos+ Utils::Polar2Vector(700,me2ball.dir());
...
//            if(isKick){
                setSubTask(PlayerRole::makeItGoto(vecNumber,getballPos,taskDir,CVector(0,0),8,3000,8,5000,5,PlayerStatus::NOT_AVOID_THEIR_VEHICLE| PlayerStatus::NOT_AVOID_PENALTY));
//            }else{
...
//            }
    ...
    if(canKick){
        if(ball.X()>PARAM::Field::PITCH_LENGTH/4){
            KickStatus::Instance()->setKick(vecNumber,6000);
        }else{
            KickStatus::Instance()->setKick(vecNumber,2800);
        }
    }
...
GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(1000,0),QString("%1").arg(shootTime).toLatin1());
