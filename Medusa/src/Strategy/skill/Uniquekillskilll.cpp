...
CUniquekillskilll::CUniquekillskilll(){
    ZSS::ZParamManager::instance()->loadParam(DEBUG_SWITCH,"Debug/Touch",false);
}
void CUniquekillskilll::plan(const CVisionModule* pVision){
...
//    }
    ...
    targetPos = ball.Pos() + Utils::Polar2Vector(200,Utils::Normalize((OUR_GOAL - ball.Pos()).dir() - 3.14/3));
    targetDir = (ball.Pos() - me.Pos()).dir();
    // add avoid ball flag
//    if(toBallDist>200){
//        taskFlag |= PlayerStatus::DODGE_BALL;
//    }
    if((me.Pos() - targetPos).mod() < 200)
    {
