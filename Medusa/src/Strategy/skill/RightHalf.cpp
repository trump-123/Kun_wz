...
CRightHalf::CRightHalf(){
     ZSS::ZParamManager::instance()->loadParam(DEBUG_SWITCH, "DEBUG/RightHalf", false);
}
void CRightHalf::plan(const CVisionModule* pVision){
    ...
    CGeoPoint TargetPos;
    double TargetDir;
    int total = 0,attack[6],assist = 0,midattack[6];
    double dist[6],middist[6];
    ...
    for(int num = 0;num<PARAM::Field::MAX_PLAYER-1;num++)
    {
        ...
        if(enemy.Valid())
        {
            if(enemy.X()<0)
            {
                ...
                {
                    dist[total] = (enemy.Pos() - OUR_GOAL).mod();
                    attack[total] = num;
                    total++;
                }
    ...
    int real = -1,j = attack[0],i = midattack[0];
    double m = dist[0],m1 = middist[0];
    ...
    if(danger_to_our_goal&&ball.Y()>0&& ball.Vel().mod() > 1500){//紧急防球
        ...
        if(attack[0]!=-1||assist>1)
        {
            ...
            if(enemy.Y()>0)
            {
                if(theirtendourgoal(enemy.Dir(),enemy.Pos()))
                {
                    ...
                    {
                        stand_pos = CGeoPoint(-PARAM::Field::PITCH_LENGTH/2 + PARAM::Field::MAX_PLAYER_SIZE/2 + PARAM::Field::PENALTY_AREA_DEPTH,PARAM::Field::PENALTY_AREA_WIDTH/2);
                    }
            ...
            else
            {
                ...
                TargetPos = CGeoPoint(-PARAM::Field::PITCH_LENGTH/2 + PARAM::Field::MAX_PLAYER_SIZE/2 + PARAM::Field::PENALTY_AREA_DEPTH,PARAM::Field::PENALTY_AREA_WIDTH/2);
            }
        }
        else
        {
