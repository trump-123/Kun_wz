...
CSpecial::CSpecial():_state(CSpecial::NOTHING){
     ZSS::ZParamManager::instance()->loadParam(DEBUG_SWITCH, "DEBUG/Special", true);
}
void CSpecial::plan(const CVisionModule* pVision){
    if(pVision->getCycle() - _lastCycle > PARAM::Vision::FRAME_RATE*0.1)
    {_state = CSpecial::NOTHING;}
...
//cout<<"0"<<endl;
    ...
    static double po5[6];//里球最近的点位
    static int num1[6];//球员号
    int i=0;
    int Validplayernum =0 ;
    for(int num=0;num<12;num++)//扫描对方车的位置和车号
    { //获取敌方有效车号
    ...
    int t2 = 0;
    static int b[6];//球员号
    for(int i = 0; i<Validplayernum ; i++)//离球最近的排序
    {
        a[i] = po5[i];
        b[i] = num1[i];
    }
    for(Pmax = 0;Pmax<Validplayernum;Pmax++)
    {
            for (Pmin = Pmax+1;Pmin<6;Pmin++)
            {
                if(a[Pmax]>a[Pmin])    //如果前一个数比后一个数大，则利用中间变量t实现两值互换
                {
                    t1=a[Pmax];
                    a[Pmax]=a[Pmin];
                    a[Pmin]=t1;
    ...
    for(int i=0;i<6;i++)
    {
        cout<<i<<"  "<<a[i]<<"  "<<b[i]<<endl;//排序显示
    }

    static int n = 0;
    double q = po5[0];
    double getballplayer = po5[0];
    static int getballplayernum = 0;
    double ed = 0;
    for(int i=0;i<Validplayernum;i++)
    {
        if(po5[i]<= getballplayer)
        {
            getballplayer = po5[i];
            getballplayernum = num1[i];
        }
    }
    if(getballplayer == a[0])
    {
        ed = a[2];//防守离球最近的球员位置排序号
        for(int i = 0;i<Validplayernum;i++)
        {
            if(ed == po5[i])
            {
    ...
    cout<<"special:"<<n<<endl;//防守离持球人最近距离的球员
    ...
    const PlayerVisionT& Apos= pVision->theirPlayer(getballplayernum);
    CGeoPoint getballPos = Apos.Pos();

   // float radian = atan2((pos1.Y()-Apos.Y()),(pos1.X()-Apos.X()));
    //float angle = radian * 360 / pi;
    auto angle1 = (Apos.Pos() - pos1.Pos()).dir();//朝向持球人的角度

    //cout<<"assister:"<<angle1<<endl;
    //防守位置：球员与球员之间的位置

    CGeoPoint defend(p + Utils::Polar2Vector(130,angle1));
...
/*
    ...
    //{
        ...
        newTask.player.pos = defend;
        newTask.player.angle = Utils::Normalize(angle1 );
