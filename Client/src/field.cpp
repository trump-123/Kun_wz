...
//    pixmapPainter.setRenderHint(QPainter::SmoothPixmapTransform, true);
    QString logoStr;
    zpm->loadParam(logoStr,"field/logo_background","1");
    _logo = QImage(QString(":/logo/%1.png").arg(logoStr));
    _logo = _logo.scaled(QSize(canvasWidth,canvasHeight));
    init();
...
//        paintInit();
//        pixmapPainter.setOpacity(0.35);
//        pixmapPainter.drawImage(0,0,_logo);
...
//            paintCarShadow(robot.pos.x(), robot.pos.y(), robot.angle);
    ...
    for(int j = 0; j < maintain.ballSize; j++) {
        ...
        paintFocus(ball.valid ? COLOR_PINK : COLOR_PINK_SHADOW, ball.pos.x(), ball.pos.y(), 500, ballFocusCount++);
    }
}
