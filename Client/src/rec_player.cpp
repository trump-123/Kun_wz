...
//        qDebug() << "FUCK DEBUG MESSAGE SIZE" <<  debugMsgs.ByteSize() << GlobalData::instance()->debugBlueMessages.size();
    ...
    //debugMsgs = recMsg.athenadebugmsgs();
    int size = debugMsgs.ByteSize();
    GlobalData::instance()->athenaDebugMessage.resize(size);
    debugMsgs.SerializeToArray(GlobalData::instance()->athenaDebugMessage.data(), size);
