...
void ActionModuleSerialVersion::sendLegacy(const ZSS::Protocol::Robots_Command& commands){
    ...
    for(int i=0;i<size;i++){
        ...
        if(id >= 0 && id < PARAM::ROBOTMAXID){
            ...
            NJ_CMDS[id].power = command.power()/100.0;//10.0
...
//    tx[num  + 21] = power;
    ...
    quint8 ipower = 0;
    if(speed > 0.5) {
        ipower = kickStandardization(real_num, kick_mode, (quint16)(power));
    }
...
quint8 kickStandardization(quint8 id, bool mode, quint16 power) {
    ...
    KParamManager::instance()->loadParam(max_power, key, 127);
